...
\hypersetup{
  ...
  linkcolor=blue,
  urlcolor=blue,
  pdftitle={WashMate Campus 产品与实现说明文档},
...
\title{WashMate Campus\\产品与实现说明文档}
...
本文档基于当前仓库已经实现的事实，说明 WashMate Campus 的产品目标、前端实现、后端参考实现、数据结构、外部服务边界、测试验证和发布链路。它不是一份设想中的未来架构说明，而是面向产品介绍、团队交接和后续维护的技术说明文档。

本项目最关键的实现事实是：WashMate Campus 不是“网页前端 + 长期运行的 Python HTTP 后端”的传统结构。当前稳定的用户侧产品是 \path{frontend/} 下的 React + TypeScript + Vite + Capacitor 移动端应用。核心业务服务以内置 TypeScript 模块的形式运行在 APK 内部，主要位于 \path{frontend/src/api/}。\path{backend/} 下的 Python 模块是参考实现和测试基准，用于明确模块边界、业务规则、数据结构和集成测试行为。
