...
- 图片选择可把用户本机上传的图片保存为衣柜缩略图，方便用户区分衣物；图片内容只保存在本地衣柜记录中，不自动上传。只有用户主动点击识图或批量识图时，才把图片内容发送到用户配置的 ModelHub endpoint。
- 衣柜页只展示当前有哪些衣物及其分类、缩略图和详情；脏衣服的选择入口在主页“脏衣篮”，点进二级页面后再选择本次哪些衣物已经放进脏衣篮。
- 脏衣篮记录使用 `item_id`、`added_at` 和 `added_at_source` 保存本地状态；新增选择写入真实加入时间，旧版字符串记录或缺失/非法时间迁移时必须标记为 `estimated` 并在 UI 中显示“加入时间待确认”，不能静默当作准确日期。
- 前端 ModelHub 图片识别和文字识别请求都必须使用 `responseMimeType=application/json` 和 `responseSchema` 约束输出 JSON；分类字段由识别结果补充，允许用户在保存前编辑。
...
- 不在源码、构建配置或 APK 中保存密钥、令牌、真实账号凭证或遥测数据；用户手动输入的 `apikey` 可保存在本机应用存储中，便于下次打开继续识图，但必须允许用户主动清除。
...
- Android APK 的 ModelHub `baseUrl` 和 `apikey` 由用户输入；前端不得内置真实账号凭证或 API key，用户输入的配置可保存在本机，清除后识图功能应回到未配置状态。
